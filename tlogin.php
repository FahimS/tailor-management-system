...
if(isset($_POST['tlogin'])){
    ...
    $query="select * from treg where email='$email' and password='$password';";
    $result=mysqli_query($conn,$query);
    $check_user = mysqli_num_rows($result);
    if($check_user==0){
            echo "<script>alert('User not found! Registration First!!!')</script>";
            echo"<script>window.open('tlogin.php','_self')</script>";
    }
      ...
      $_SESSION['email']=$email;
        while($row = mysqli_fetch_array($result)){
            ...
            echo "<script>window.open('tms_index.php','_self')</script>";
                ...
                echo "<script>alert('Your account is Disable!!!  Please, Contact with Admin.')</script>";
        echo"<script>window.open('tlogin.php','_self')</script>";
            }
        }
    }
      ...
      <div class="right templete clear">
        <div class="formbox templete clear">
          <form action="tlogin.php" method="post">
            ...
            <input  type="submit" name="tlogin" value="Sign In" >
            <!-- <a href="tms_index.php"><p><input type="submit" name="tlogin" value="Sign In"/></p></a> -->
          </form>
        </div>
      </div>
