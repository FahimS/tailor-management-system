...
?>
    ...
    <nav class="navbar navbar-default">
        <div class="container">
            <div class="navbar-header">
              ...
              <a class="navbar-brand" href="tms_index.php"><img src="image/809HRD.png" alt="" height="50"></a>
            </div>
    ...
    <section id="main">
        <div class="container">
            <div class="row">
                ...
                <!------- update pending list of order information -------->
                <div class="col-md-9">
                    <div class="panel panel-default ">
                      ...
                      <div class="panel-body" id="divToPrint">
                          <!------- php code for transaction id showing information of pending list  -------->
                          <?php
                            if(isset($_GET['trans_id'])){
                                $trans_id = $_GET['trans_id'];
                                $get_info = "select * from order_info where tran_id='$trans_id'";
                                $run_pro = mysqli_query($conn, $get_info); 
                                while($row_pro=mysqli_fetch_array($run_pro)){
                                $tran_id = $row_pro['tran_id'];
                                $voucher = $row_pro['voucher'];
                                $tran_id = $row_pro['tran_id'];
                                $name = $row_pro['customer_name'];
                                $phone = $row_pro['customer_phone'];
                          ...
                          ?> 
                          <form action="update_pending.php?trans_id=<?php echo $tran_id?>" method="post" enctype="multipart/form-data">
                              <table align="center" width="500" border="2">
                                  <tr>
                                      <td colspan="8" ><h2 align="center">Payment Voucher</h2><br></td>
                                  </tr>
                                  <tr>
                                      <td colspan="8" ><h4 align="center">Pricing</h4></td>
                                  </tr>
                                  <tr>
                                  ...
                                  <tr>
                                      <td align="center" width="250" height="50">
                                          ...
                                          <input type="date" id="start" name="order_date" min="2018-01-01" value="<?php echo $order_date ?>">
                                      </td>
                                      <td align="center" width="250" height="50">
                                          <label >Delivery Date</label>
                                          <input type="date" id="end" name="delivery_date" min="2018-01-01" value="<?php echo $delivery_date ?>">
                                      </td>
                                  </tr>
                                  <tr>
                                      <td colspan="8" ><h4 align="center">Customer Address</h4></td>
                                  </tr>
                                  <tr>
                             ...
                             <div>
                                 <input type="submit" class="custom-button-account" name="update_order" value="save change"><!--  this button update list -->
                       
                                 <input type="submit" class="custom-button-account" name="insert_complete" value="Complete"><!--  this button update pending to complete list -->
                             </div>
  ...
  <div class="social2">
    ...
    <!--  php code for update order details -->
    <?php
        if(isset($_POST['update_order'])){
            ...
            /*>>>>>>>>>>>>>>>>>this query update order_info table<<<<<<<<<<<<<<<*/
            $update= "UPDATE order_info SET customer_name='$customer_name', customer_phone = '$customer_phone', customer_address = '$customer_address', total_price='$total_price', paid = '$paid', due='$due', delivery_date='$delivery_date' where tran_id = '$trans_id' ";
            $insert_pass = mysqli_query($conn,$update);
            if($insert_pass){
                echo "<script>alert('Order update successfully!')</script>";
                echo "<script>window.open('pending_list.php','_self')</script>";
            }
        }
    ?>
    <!--  php code for change order from pending list to complete list -->
    <?php
        if(isset($_POST['insert_complete'])){
            ...
            $delivery_date = $_POST['delivery_date'];
            $status = $_POST['status'];
            /*>>>>>>>>>>>>>>>>>this query insert product in complete list table<<<<<<<<<<<<<<<*/
            $insert_product = "insert into complete_list(tran_id,voucher_no,customer_name,category,delivery_date,total) select tran_id,voucher,customer_name,category,delivery_date,total_price from order_info where tran_id='$trans_id';";
            /*>>>>>>>>>>>>>>>>>this query update status of order_info table<<<<<<<<<<<<<<<*/
            $insert_product .= "UPDATE order_info SET status='$status' where tran_id='$trans_id'";
            
            if($conn->multi_query($insert_product) === true){
                echo "<script>alert('Order updated successfully!')</script>";
                echo "<script>window.open('complete_list.php','_self')</script>";
            }
        }
    ?>
