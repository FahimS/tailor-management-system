...
if(isset($_GET['rs_id']))
    ...
    if($opr=="del")
{
        $query="DELETE FROM creg WHERE id=$id";
...
<div class="view_admin_darjibari">
    ...
    if($key !="")
    {
        $query="SElECT * FROM creg WHERE name  like '%$key%' ";
        $sql_sel=mysqli_query($con,$query);
    }
        
    else
    {
        $query="SELECT * FROM creg";
        $sql_sel=mysqli_query($con,$query);
    }
    ...
    ?>
      <tr bgcolor="<?php echo $color?>">
          ...
          <td><?php echo $row['email'];?></td>
           ...
           <td><a href="update_customer.php?edit_id=<?php echo $row['id'];?>" title="Update"><img style="width: 30px;" src="image/updatesicon.png" /></a></td>
